...
str_to_cpu_mode(const std::string &cpu_mode) {
  ...
  std::transform(cpu_mode.begin(), cpu_mode.end() + 1, upper_key.begin(),
                 ::toupper);*/
  // 这里原本是将传入的 cpu_mode 做了一个转换成全大写的操作，但是不知道为什么，转换后就会 find 不到这个 key，所以这里索性把转换去掉了
  // 因此需要保证传入的 cpu_mode 是全大写的，不然依旧会查找失败
...
Java_com_equationl_paddleocr4android_Util_paddle_OCRPredictorNative_forward(
...
Java_com_equationl_paddleocr4android_Util_paddle_OCRPredictorNative_release(
